...
const TripsScreen: React.FC = () => {
  ...
  const fetchEstimatedTime = async (origin: string, destination: string) => {
    try {
      const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}&key=${GOOGLE_MAPS_APIKEY}`;
  ...
  const handleSave = async () => {
    ...
    estimatedTime = await fetchEstimatedTime(form.start, form.end);
    if (editingTrip) {
      setTrips(trips.map(t => t.id === editingTrip.id ? { ...editingTrip, ...form, estimatedTime } : t));
    } else {
      setTrips([
        ...trips,
        {
