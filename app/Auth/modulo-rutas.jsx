...
const App = () => {
  ...
  const requestLocationPermission = async () => {
    try {
      ...
      showToast('Error al solicitar permisos de ubicación');
      return false;
    }
  };
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (address) {
        ...
        setAddressStart({
          address: formattedAddress || "Ubicación actual",
  ...
  const handleLocationError = (error) => {
    ...
    if (error.code === 1) {
      message = 'Por favor habilita los permisos de ubicación en la configuración de tu dispositivo';
  ...
  // Función para obtener actualizaciones de ubicación en tiempo real
  const startLocationUpdates = async () => {
    try {
      const { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== 'granted') {
        showToast('Se requieren permisos de ubicación para actualizaciones en tiempo real');
        return;
      }
      ...
      await Location.watchPositionAsync(
        ...
        (location) => {
          ...
          // Actualizar solo si la ubicación ha cambiado significativamente
          if (markers.length > 0) {
            const lastLocation = markers[0];
            ...
            if (distance > 0.01) { // 10 metros
              setMarkers(prevMarkers => [newLocation, ...(prevMarkers.slice(1))]);
      ...
      console.error('Error al iniciar actualizaciones de ubicación:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    // Limpiar suscripción a actualizaciones de ubicación
    return () => {
      // La limpieza se maneja automáticamente por expo-location
    };
  }, []);
